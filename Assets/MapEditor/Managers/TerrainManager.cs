...
public static class TerrainManager
{
    ...
    // Resolution
    public static int HeightMapRes { get; set; }
    public static int SplatMapRes { get; set; }
	...
	public static Texture2D SlopeTexture;
	public static Texture2D CurveTexture;
	
	public static Texture2D BlendMapTexture;
	public static Texture2D MaskTexture;
	public static Texture2D AlphaBuffer;
	...
	public static void RuntimeInit()
	{
		...
		Debug.Log("callbacks setup");
		
		SetTerrainReferences();
		Debug.Log("references set");
		
		SyncTerrainResolutions();		
		Debug.Log("resolutions detected");
		
		PopulateTerrainArrays();
		Debug.Log("arrays populated");
				
		FilterTexture = Resources.Load<Texture>("Textures/Brushes/White128");
		Debug.Log("filter texture applied");
	...
	public static void OnBundlesLoaded()
	{
		...
		try
		{
			...
			if (AssetManager.AreBundlesLoaded())
			{
				...
				LoadTerrainAssets();
				Debug.Log("Terrain assets loaded");
				ConfigureShaderGlobals(Land);
				Debug.Log("Shader globals configured");
		...
		{
			Debug.LogError($"Error in OnBundlesLoaded: {ex.Message}\n{ex.StackTrace}");
		}

		LoadScreen.Instance.SetMessage("Bundles loaded");
		LoadScreen.Instance.Complete(0);
	}
	...
	public static Layers CreateLayerFromKeyword(string keyword)
	{
...
public static void SyncAlphaTexture()
{
    ...
    // Initialize AlphaBuffer if needed
    if (AlphaBuffer == null || AlphaBuffer.width != AlphaMapRes || AlphaBuffer.height != AlphaMapRes)
    {
        AlphaBuffer = new Texture2D(AlphaMapRes, AlphaMapRes, TextureFormat.R8, false);
        AlphaBuffer.filterMode = FilterMode.Point;
        AlphaBuffer.wrapMode = TextureWrapMode.Clamp;
    }

    // Initialize AlphaTexture if needed
    if (AlphaTexture == null || AlphaTexture.width != AlphaMapRes || AlphaTexture.height != AlphaMapRes)
    {
        AlphaTexture = new RenderTexture(AlphaMapRes, AlphaMapRes, 0, RenderTextureFormat.R8);
        AlphaTexture.filterMode = FilterMode.Point;
        AlphaTexture.wrapMode = TextureWrapMode.Clamp;
        AlphaTexture.Create();
    }

    byte[] pixels = new byte[AlphaMapRes * AlphaMapRes];
    // Populate pixels
    Parallel.For(0, AlphaMapRes, z =>
    {
        for (int x = 0; x < AlphaMapRes; x++)
        {
            pixels[z * AlphaMapRes + x] = Alpha[z, x] ? (byte)255 : (byte)0;
        }
    });

    // Update AlphaBuffer
    AlphaBuffer.SetPixelData(pixels, 0);
    AlphaBuffer.Apply(false, false); // Set to false if readback isn’t needed

    // Blit to AlphaTexture
    Graphics.Blit(AlphaBuffer, AlphaTexture);
	...
	public static void SyncHeightSlopeCurveTexture(int targetResolution)
    {
...
{
    if (Biome == null || Biome.GetLength(0) != SplatMapRes || Biome.GetLength(1) != SplatMapRes || Biome.GetLength(2) != 5)
    {
    ...
    if (BiomeTexture == null || BiomeTexture.width != SplatMapRes || BiomeTexture.height != SplatMapRes || !BiomeTexture.IsCreated())
    {
        Debug.LogWarning("BiomeTexture not properly initialized. Re-initializing.");
        InitializeTextures();
    }
    if (Biome1Texture == null || Biome1Texture.width != SplatMapRes || Biome1Texture.height != SplatMapRes || !Biome1Texture.IsCreated())
    {
    ...
    // Create a single temporary texture for both updates
    ...
    for (int z = 0; z < SplatMapRes; z++)
    {
        ...
        {
            int index = z * SplatMapRes + x;
            colors[index] = new Color32(
                (byte)(Mathf.Clamp01(Biome[z, x, 0]) * 255), // Arid
                (byte)(Mathf.Clamp01(Biome[z, x, 1]) * 255), // Temperate
                (byte)(Mathf.Clamp01(Biome[z, x, 2]) * 255), // Tundra
                (byte)(Mathf.Clamp01(Biome[z, x, 3]) * 255)  // Arctic
            );
        }
    }
    ...
    for (int z = 0; z < SplatMapRes; z++)
    {
        for (int x = 0; x < SplatMapRes; x++)
        {
            int index = z * SplatMapRes + x;
            colors[index] = new Color32(
                (byte)(Mathf.Clamp01(Biome[z, x, 4]) * 255), // Jungle
                0, 0, 255
            );
        }
    }
    tempTexture.SetPixels32(colors);
    tempTexture.Apply(false, false);
    Graphics.Blit(tempTexture, Biome1Texture);
	...
	{
		Debug.Log("loading terrain assets");
		_config = Resources.Load<TerrainConfig>("TerrainConfig");
		...
		{
			Debug.LogError("terrain4 prefab not loaded");
			return;
		}
		CreateTerrainConfig();
		Debug.Log("terrain config created");
...
private static void ConfigureShaderGlobals(Terrain terrain)
{
	...
	public static float[,,] GetBiomeMap(int startX, int startY, int width, int height)
	{
			if (Biome == null)
			{
				Debug.LogWarning("GetBiomeMap (~undo): Failed to retrieve biome data");
				return new float[height, width, 5];
			}
		
		// Extract region from Biome array		
		float[,,] biomeRegion = new float[height, width, 5];
		for (int i = 0; i < height; i++)
		{
			for (int j = 0; j < width; j++)
			{
				for (int k = 0; k < 5; k++)
				{
					biomeRegion[i, j, k] = Biome[startY + i, startX + j, k];
				}
			}
		}
	...
	public static void SetBiomeMap(float[,,] biomeData, int startX, int startY, int width, int height)
	{
		// Register undo for the region
		float[,,] biomeDataBefore = GetBiomeMap(startX, startY, width, height);
		
		// Update Biome array with new data
		for (int i = 0; i < height; i++)
		{
			for (int j = 0; j < width; j++)
			{
				for (int k = 0; k < 5; k++)
				{
					Biome[startY + i, startX + j, k] = biomeData[i, j, k];
				}
			}
		}
		...
		SyncBiomeTexture();
		//LayerDirty = false;
		//Callbacks.InvokeLayerUpdated(LayerType.Biome, 0);
	}
...
Callbacks.InvokeHeightMapUpdated(TerrainType.Land);
	...
	public static bool[,] GetAlphaMap(int x, int y, int width, int height)
	{
		...
		if (AlphaDirty)
		{
			Debug.LogError("shouldnt be blowing out the alpha");
	...
	public static void SetCliffMap(bool[,] spawns)	{	SpawnMap = spawns;	}
...
public static bool[,] UpscaleBitmap(bool[,] source)
{
	...
	{
		Land.materialTemplate.SetFloat("_LandMask", 1.0f);   

	}
	
	public static void HideLandMask()
	{
		Land.materialTemplate.SetFloat("_LandMask", 0.0f);
	}
	...
	public static void SetLayerData(float[,,] array, LayerType layer, int topology = -1)
	{
		...
		switch (layer)
		{
			...
			case LayerType.Biome:
				Biome = array;
				//RegisterBiomeMapUndo($"Set {layer} Layer Array");
	...
	public static void SetAlphaMap(bool[,] array, int x, int y, int width, int height)
	{
		...
		if (Alpha == null || Alpha.GetLength(0) != AlphaMapRes || Alpha.GetLength(1) != AlphaMapRes)
		{
			Debug.LogError("making a new array");
			Alpha = new bool[AlphaMapRes, AlphaMapRes];
		}
	...
	public static float[,] GetHeightMap(int startX, int startY, int width, int height, TerrainType terrain = TerrainType.Land)
    {
        TerrainData terrainData = terrain == TerrainType.Land ? Land?.terrainData : Water?.terrainData;
        if (terrainData == null)
        {
            Debug.LogError($"TerrainManager.GetHeightMap: {(terrain == TerrainType.Land ? "Land" : "Water")} terrainData is null");
...
public static void SetHeightMapRegion(float[,] array, int x, int y, int width, int height, TerrainType terrain = TerrainType.Land)
{
	...
	public static void SyncHeightTextures()
	{
		SyncTerrainResolutions();
		// Validate arrays and textures
		if (Height == null || Height.GetLength(0) != HeightMapRes || Height.GetLength(1) != HeightMapRes)
		{
			Debug.LogError($"Height array is null or has incorrect dimensions: Expected {HeightMapRes}x{HeightMapRes}, Got {(Height != null ? $"{Height.GetLength(0)}x{Height.GetLength(1)}" : "null")}");
			return;
		}
		if (HeightTexture == null || HeightTexture.width != HeightMapRes || HeightTexture.format != TextureFormat.R16)
		{
			Debug.LogError($"HeightTexture is invalid: Expected {HeightMapRes}x{HeightMapRes} R16, Got {(HeightTexture != null ? $"{HeightTexture.width}x{HeightTexture.height} {HeightTexture.format}" : "null")}");
			return;
		}

		if (Slope == null || Slope.GetLength(0) != HeightMapRes || Slope.GetLength(1) != HeightMapRes)
		{
			Debug.LogError($"Slope array is null or has incorrect dimensions: Expected {HeightMapRes}x{HeightMapRes}, Got {(Slope != null ? $"{Slope.GetLength(0)}x{Slope.GetLength(1)}" : "null")}");
			return;
		}
		if (SlopeTexture == null || SlopeTexture.width != HeightMapRes || SlopeTexture.format != TextureFormat.R16)
		{
			Debug.LogError($"SlopeTexture is invalid: Expected {HeightMapRes}x{HeightMapRes} R16, Got {(SlopeTexture != null ? $"{SlopeTexture.width}x{SlopeTexture.height} {SlopeTexture.format}" : "null")}");
			return;
		}

		if (Curvature == null || Curvature.GetLength(0) != HeightMapRes || Curvature.GetLength(1) != HeightMapRes)
		{
			Debug.LogError($"Curvature array is null or has incorrect dimensions: Expected {HeightMapRes}x{HeightMapRes}, Got {(Curvature != null ? $"{Curvature.GetLength(0)}x{Curvature.GetLength(1)}" : "null")}");
			return;
		}
		if (CurveTexture == null || CurveTexture.width != HeightMapRes || CurveTexture.format != TextureFormat.R16)
		{
			Debug.LogError($"CurveTexture is invalid: Expected {HeightMapRes}x{HeightMapRes} R16, Got {(CurveTexture != null ? $"{CurveTexture.width}x{CurveTexture.height} {CurveTexture.format}" : "null")}");
			return;
		}

		// Allocate pixel arrays
		Color[] heightPixels = new Color[HeightMapRes * HeightMapRes];
		Color[] slopePixels = new Color[HeightMapRes * HeightMapRes];
		Color[] curvePixels = new Color[HeightMapRes * HeightMapRes];

		// Sync HeightTexture in parallel
		Parallel.For(0, HeightMapRes, i =>
		{
			for (int j = 0; j < HeightMapRes; j++)
			{
				float value = Mathf.Clamp01(Height[i, j]); // Ensure 0-1 range
				heightPixels[i * HeightMapRes + j] = new Color(value, 0, 0); // Red channel for R16
			}
		});
		HeightTexture.SetPixels(heightPixels);
		HeightTexture.Apply();
		Shader.SetGlobalTexture("Terrain_HeightTexture", HeightTexture);
		Debug.Log($"Synced HeightTexture with resolution {HeightMapRes}x{HeightMapRes}, Format: {HeightTexture.format}");

		// Sync SlopeTexture in parallel
		Parallel.For(0, HeightMapRes, i =>
		{
			for (int j = 0; j < HeightMapRes; j++)
			{
				float value = Mathf.Clamp01(Slope[i, j] / 90f); // Normalize slope (0-90 degrees) to 0-1
				slopePixels[i * HeightMapRes + j] = new Color(value, 0, 0); // Red channel for R16
			}
		});
		SlopeTexture.SetPixels(slopePixels);
		SlopeTexture.Apply();
		Shader.SetGlobalTexture("Terrain_SlopeTexture", SlopeTexture);
		Debug.Log($"Synced SlopeTexture with resolution {HeightMapRes}x{HeightMapRes}, Format: {SlopeTexture.format}");

		// Sync CurveTexture in parallel
		Parallel.For(0, HeightMapRes, i =>
		{
			for (int j = 0; j < HeightMapRes; j++)
			{
				float value = Mathf.Clamp01(Curvature[i, j]); // Ensure 0-1 range
				curvePixels[i * HeightMapRes + j] = new Color(value, 0, 0); // Red channel for R16
			}
		});
		CurveTexture.SetPixels(curvePixels);
		CurveTexture.Apply();
		Shader.SetGlobalTexture("Terrain_CurveTexture", CurveTexture);
		Debug.Log($"Synced CurveTexture with resolution {HeightMapRes}x{HeightMapRes}, Format: {CurveTexture.format}");
	}
	...
	public static void InitializeTextures()
		{
			...
			{
				Debug.Log("destroying alpha texture of width:" + AlphaTexture.width + " which was expected to be " + AlphaMapRes);
				UnityEngine.Object.Destroy(AlphaTexture);
				AlphaTexture = null;
			}
			if (AlphaTexture == null)
			{
				AlphaTexture = new RenderTexture(AlphaMapRes, AlphaMapRes, 0, RenderTextureFormat.R8, RenderTextureReadWrite.Linear)
				{
					wrapMode = TextureWrapMode.Clamp,
					enableRandomWrite = true,
					name = "Terrain_Alpha"
				...
				AlphaTexture.Create();
				Debug.Log($"Initialized AlphaTexture with resolution {AlphaMapRes}x{AlphaMapRes}, Format: {AlphaTexture.format}");
			}

			// Initialize AlphaBuffer
			if (AlphaBuffer != null && (AlphaBuffer.width != AlphaMapRes || AlphaBuffer.format != TextureFormat.R8))
			{
				UnityEngine.Object.Destroy(AlphaBuffer);
				AlphaBuffer = null;
			}
			if (AlphaBuffer == null)
			{
				AlphaBuffer = new Texture2D(AlphaMapRes, AlphaMapRes, TextureFormat.R8, false, true)
				{
					wrapMode = TextureWrapMode.Clamp,
					filterMode = FilterMode.Point, // Point filtering for binary data
					name = "Terrain_AlphaBuffer"
				};
				Debug.Log($"Initialized AlphaBuffer with resolution {AlphaMapRes}x{AlphaMapRes}, Format: {AlphaBuffer.format}, Wrap: {AlphaBuffer.wrapMode}, Filter: {AlphaBuffer.filterMode}");
			}
			...
			{
				BlendMapTexture = new Texture2D(SplatMapRes, SplatMapRes, TextureFormat.RGBA32, false, true)
				{
					wrapMode = TextureWrapMode.Clamp,
					filterMode = FilterMode.Bilinear,
					name = "Terrain_BlendMap"
				};
				// Clear to fully transparent (no blending, RGBA = 0,0,0,0)
				Color[] pixels = new Color[SplatMapRes * SplatMapRes];
				for (int i = 0; i < pixels.Length; i++)
				{
					pixels[i] = new Color(0f, 0f, 0f, 0f); // Set alpha to 0
				}
			...
			if (MaskTexture != null && (MaskTexture.width != HeightMapRes || MaskTexture.format != TextureFormat.R8))
			{
			...
			if (MaskTexture == null)
			{
				MaskTexture = new Texture2D(HeightMapRes, HeightMapRes, TextureFormat.R8, false, true)
				{
					wrapMode = TextureWrapMode.Clamp,
					filterMode = FilterMode.Point, // Point filtering for binary mask data
					name = "Terrain_MaskTexture"
				};

				Shader.SetGlobalTexture("Terrain_Preview", MaskTexture);
				Debug.Log($"Initialized MaskTexture with resolution {HeightMapRes}x{HeightMapRes}, Format: {MaskTexture.format}, Wrap: {MaskTexture.wrapMode}, Filter: {MaskTexture.filterMode}");
			}
			
			// Initialize HeightTexture
			if (HeightTexture != null && (HeightTexture.width != HeightMapRes || HeightTexture.format != TextureFormat.R16))
			{
			...
			{
				HeightTexture = new Texture2D(HeightMapRes, HeightMapRes, TextureFormat.R16, false, true)
				{
					wrapMode = TextureWrapMode.Clamp,
					filterMode = FilterMode.Point,
					name = "Terrain_HeightTexture"
				};
				Shader.SetGlobalTexture("Terrain_HeightTexture", HeightTexture);
				Debug.Log($"Initialized HeightTexture with resolution {HeightMapRes}x{HeightMapRes}, Format: {HeightTexture.format}, Wrap: {HeightTexture.wrapMode}, Filter: {HeightTexture.filterMode}");
			}

			// Initialize SlopeTexture
			if (SlopeTexture != null && (SlopeTexture.width != HeightMapRes || SlopeTexture.format != TextureFormat.R16))
			{
				UnityEngine.Object.Destroy(SlopeTexture);
				SlopeTexture = null;
			}
			if (SlopeTexture == null)
			{
				SlopeTexture = new Texture2D(HeightMapRes, HeightMapRes, TextureFormat.R16, false, true)
				{
					wrapMode = TextureWrapMode.Clamp,
					filterMode = FilterMode.Point,
					name = "Terrain_SlopeTexture"
				};
				Shader.SetGlobalTexture("Terrain_SlopeTexture", SlopeTexture);
				Debug.Log($"Initialized SlopeTexture with resolution {HeightMapRes}x{HeightMapRes}, Format: {SlopeTexture.format}, Wrap: {SlopeTexture.wrapMode}, Filter: {SlopeTexture.filterMode}");
			}

			// Initialize CurveTexture
			if (CurveTexture != null && (CurveTexture.width != HeightMapRes || CurveTexture.format != TextureFormat.R16))
			{
				UnityEngine.Object.Destroy(CurveTexture);
				CurveTexture = null;
			}
			if (CurveTexture == null)
			{
				CurveTexture = new Texture2D(HeightMapRes, HeightMapRes, TextureFormat.R16, false, true)
				{
					wrapMode = TextureWrapMode.Clamp,
					filterMode = FilterMode.Point,
					name = "Terrain_CurveTexture"
				};
				Shader.SetGlobalTexture("Terrain_CurveTexture", CurveTexture);
				Debug.Log($"Initialized CurveTexture with resolution {HeightMapRes}x{HeightMapRes}, Format: {CurveTexture.format}, Wrap: {CurveTexture.wrapMode}, Filter: {CurveTexture.filterMode}");
			}
...
public static void ChangeLayer(LayerType layer, int topology = -1)
{
	...
	public static void ClearUndo()	{
		TerrainUndoManager.ClearHistory();
	}
