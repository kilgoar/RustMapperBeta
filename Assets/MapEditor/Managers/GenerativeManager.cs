...
{
	private static bool[,] spawnMap;
    private static Color[] maskPixels;
	private static List<GeologySpawner> spawners = new List<GeologySpawner>();
	...
	public static void InitializeReusableArrays()
    {
        if (TerrainManager.HeightMapRes <= 0)
        {
            Debug.LogWarning("HeightMapRes is not set. Arrays will be initialized later.");
            return;
        }

        // Allocate arrays only if not already allocated or if resolution changed
        if (spawnMap == null || spawnMap.GetLength(0) != TerrainManager.HeightMapRes)
        {
            spawnMap = new bool[TerrainManager.HeightMapRes, TerrainManager.HeightMapRes];
            maskPixels = new Color[TerrainManager.HeightMapRes * TerrainManager.HeightMapRes];
        }
...
private static void ApplyLayerBlend(float[,,] layerMap, int x, int z, int layerIndex, int layerCount, float strength)
{
	...
	[ConsoleCommand("viewcostmap")]
	public static void ShowCostMap()
	{
		PathManager.CreateRoadCostmap();
		TerrainManager.ShowLandMask();
	}
	
	[ConsoleCommand("generate roads")]
	public static void GenerateRoads(int maxConnections = 50)
	{
		// Retrieve all road points
		Dictionary<Vector3, List<Vector3>> roadPositions = PathManager.RetrieveRoadPositions();
		
		if (roadPositions == null || roadPositions.Count < 2)
		{
			Debug.LogError("Insufficient connection points to generate roads.");
			return;
		}
		
		Debug.Log("success retrieving roads");
		
		// Filter out monuments with no valid points
		List<Vector3> monuments = roadPositions.Keys.Where(key => roadPositions[key] != null && roadPositions[key].Count > 0).ToList();
		if (monuments.Count < 2)
		{
			Debug.LogError("Fewer than two monuments have valid connection points.");
			return;
		}

		// Cap maxConnections to the maximum possible unique pairs
		int maxPossiblePairs = (monuments.Count * (monuments.Count - 1)) / 2; // Number of unique monument pairs
		maxConnections = Mathf.Min(maxConnections, maxPossiblePairs);
		Debug.Log($"Generating up to {maxConnections} road connections for {monuments.Count} monuments.");

		// List to store pairs of points to connect
		List<(Vector3 startPos, Vector3 endPos)> pairsToConnect = new List<(Vector3, Vector3)>();

		// Track used monument pairs to avoid duplicates
		HashSet<(Vector3, Vector3)> usedPairs = new HashSet<(Vector3, Vector3)>();

		// Iterate over monuments to generate unique pairs
		for (int i = 0; i < monuments.Count; i++)
		{
			if (pairsToConnect.Count >= maxConnections)
			{
				break;
			}

			Vector3 startMonument = monuments[i];
			List<Vector3> startPositions = roadPositions[startMonument];
			if (startPositions.Count == 0)
			{
				continue; // Skip if no points available
			}

			for (int j = i + 1; j < monuments.Count; j++) // Start from i+1 to avoid reciprocal pairs
			{
				if (pairsToConnect.Count >= maxConnections)
				{
					break;
				...
				Vector3 endMonument = monuments[j];
				List<Vector3> endPositions = roadPositions[endMonument];
				if (endPositions.Count == 0)
				{
					continue; // Skip if no points available
				}

				// Get random points from each monument's point list
				int startIndex = UnityEngine.Random.Range(0, startPositions.Count);
				int endIndex = UnityEngine.Random.Range(0, endPositions.Count);
				Vector3 startPos = startPositions[startIndex];
				Vector3 endPos = endPositions[endIndex];

				// Check if this monument pair has been used (in either direction)
				var pair = (startMonument, endMonument);
				var reversePair = (endMonument, startMonument);
				if (usedPairs.Contains(pair) || usedPairs.Contains(reversePair))
				{
					continue; // Skip if pair already used
				}

				// Add the pair to connect
				pairsToConnect.Add((startPos, endPos));
				usedPairs.Add(pair); // Mark pair as used

				// Log the pair for debugging
				Debug.Log($"Added road connection from monument {startMonument} to {endMonument} (points: {startPos} to {endPos})");
			}
		}

		// Generate roads for each pair
		int successfulRoads = 0;
		foreach (var pair in pairsToConnect)
		{
			string roadName = $"Road {PathManager._roadIDCounter++}";

			GameObject roadObject = PathManager.GenerateRoadWithNodes(pair.startPos, pair.endPos, roadName, 10);
			if (roadObject != null)
			{
				successfulRoads++;
				Debug.Log($"Generated road '{roadName}' from {pair.startPos} to {pair.endPos}");
			}
			else
			{
				Debug.LogWarning($"Failed to generate road '{roadName}' from {pair.startPos} to {pair.endPos}");
			}
		}

		Debug.Log($"Generated {successfulRoads} of {pairsToConnect.Count} requested roads connecting road points.");
...
public static void Dir(){
    ...
    Dictionary<string, bool> uniquePaths = new Dictionary<string, bool>();
    
    // Scan dictionary for path listing without duplicates
    foreach (uint id in AssetManager.IDLookup.Keys.OrderBy(k => k))
    {
        if (AssetManager.IDLookup.TryGetValue(id, out string prefabPath) && prefabPath.EndsWith(".prefab"))
        {
            // Find last '/' to get directory path
            int lastSlashIndex = prefabPath.LastIndexOf('/');
            if (lastSlashIndex >= 0)
            {
                string directoryPath = prefabPath.Substring(0, lastSlashIndex);
                
                // Add to dictionary if not already present
                if (!uniquePaths.ContainsKey(directoryPath))
                {
                    uniquePaths.Add(directoryPath, true);
                }
            }
        }
    }
    ...
    // Log unique paths
    foreach (string path in uniquePaths.Keys)
    {
        Debug.Log(path);
    }
}

[ConsoleCommand("fill map with every prefab")]
public static void AssetDumpMap()
{
    // Define grid parameters
    const int gridSizeX = 110; // Columns
    const int gridSizeZ = 110; // Rows
    const float spacing = 30f; // Spacing between prefabs
    Vector3 startPos = new Vector3(-1000, 0, -1000); // Center the grid in 4000x4000 area

    int prefabCount = 0;
    Vector3 gridPos = startPos;

    // Get all prefab IDs and sort them
    foreach (uint id in AssetManager.IDLookup.Keys.OrderBy(k => k))
    {
        // Try to get the value associated with the ID
        if (AssetManager.IDLookup.TryGetValue(id, out string prefabPath))
        {
			// Skip paths that don't end with .prefab (case-insensitive) or contain specific substrings
			if (!prefabPath.EndsWith(".prefab", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("/ui", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("/scenes", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("/third party", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("/fx/", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("/effects/", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("/media/", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("/voiceaudio/", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("/rust.ai/", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("/sound/", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("/gamemodes/", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("/benchmark", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("storage/", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("physicstesting/", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("player/", StringComparison.OrdinalIgnoreCase) ||				
				prefabPath.Contains("/engine/", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("/subents/", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("/rust.ui/", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("/workshop/", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("/photo/", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("/debug/", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("missions/", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("slotpayoutwidget", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("rcbookmarkentry", StringComparison.OrdinalIgnoreCase) ||				
				prefabPath.Contains("notepanel", StringComparison.OrdinalIgnoreCase) ||		
				prefabPath.Contains("camera", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("driver", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("/world/", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("/seats/", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("/ch47/", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("/m2bradley/", StringComparison.OrdinalIgnoreCase) ||
				prefabPath.Contains("monumentsign", StringComparison.OrdinalIgnoreCase))
			{
				//Debug.Log("skipping " + prefabPath);
				continue;
			}
			Debug.Log("attempting to place" + prefabPath);
            try
            {
				GeologyItem item = new GeologyItem(id);
                // Create prefab at current grid position
                spawnGeoItem(item, gridPos, Vector3.zero, Vector3.one, PrefabManager.PrefabParent);
                prefabCount++;
            }
            catch (Exception e)
            {
                // Log error for the specific prefab ID
                Debug.Log($"Error placing prefab ID {id} ({prefabPath}): {e.Message}");
                continue; // Skip to the next prefab
            }
    ...
    // Post success message with actual count
    Debug.Log($"Successfully placed {prefabCount} prefabs");
}
	
	
[ConsoleCommand("fill map with every spawnable entity")]
public static void EntityDumpMap()
{
    // Define grid parameters
    const int gridSizeX = 110; // Columns
    const int gridSizeZ = 110; // Rows
	...
	public static class Coroutines
    {
		...
		public static bool makeCliffMapRunning = false;
		...
		public static IEnumerator ApplyGeologyPreset(GeologyPreset geo)
		{
				...
				yield return new WaitUntil(() => !makeCliffMapRunning);
				
				makeCliffsRunning = true;
				GenerativeManager.MakeCliffs(geo);
				yield return new WaitUntil(() => !makeCliffsRunning);
		...
		public static void SortSpawners(GeologyPreset geo)
		{
			Debug.Log(geo.hAscend + "- ascending " + geo.hDescend + "- descending");
			Debug.Log("sorting spawners");
			spawners.Sort((a, b) =>
			{
				// Sort by curvature first
				if (geo.cAscend)
				{
					//Debug.Log("sorting by curves ascending");
					int curveCompare = a.curve.CompareTo(b.curve);
					if (curveCompare != 0) return curveCompare;
				}
				else if (geo.cDescend)
				{
					//Debug.Log("sorting by curves descending");
					int curveCompare = b.curve.CompareTo(a.curve);
					if (curveCompare != 0) return curveCompare;
				}
				...
				if (geo.sAscend)
				{
					//Debug.Log("sorting by slope ascending");
					int slopeCompare = a.slope.CompareTo(b.slope);
					if (slopeCompare != 0) return slopeCompare;
				}
				else if (geo.sDescend)
				{
					//Debug.Log("sorting by slope descending");
					int slopeCompare = b.slope.CompareTo(a.slope);
					if (slopeCompare != 0) return slopeCompare;
				}
				...
				if (geo.hAscend)
				{
					//Debug.Log("sorting by height ascending");
					return a.height.CompareTo(b.height);
				}
				else if (geo.hDescend)
				{
					//Debug.Log("sorting by height descending");
	...
	public static IEnumerator MakeCliffs(GeologyPreset geo)
	{
		ClearRayDataCache();
		makeCliffsRunning = true;
		GameObject dummy = new GameObject("TempRotationObject");
		...
		float resRatio = (1f * res) / (1f * splatRes);
		string prefabName ="none";

		Dictionary<string, int> count = new Dictionary<string, int>();
		Dictionary<string, int> cullcount = new Dictionary<string, int>();
    ...
    {
        int i = (int)(spawner.i / resRatio);
        int j = (int)(spawner.j / resRatio);
            ...
            Vector3 prePosition = new Vector3((j * ratio) - (size / 2f)+halfPixelSize, (height * sizeZ) - (sizeZ * 0.5f), (i * ratio) - (size / 2f)+halfPixelSize);
			...
			if(!geo.closeOverlap){
				...
				float scale = UnityEngine.Random.Range(scaleRange1.x, scaleRange2.x);
				rScale = new Vector3(
					scale,
					scale,
					scale
			...
			jRotate.x = UnityEngine.Random.Range(rotationRange1.x, rotationRange2.x) + flipX;
			jRotate.y = UnityEngine.Random.Range(rotationRange1.y, rotationRange2.y) + geology;
			jRotate.z = UnityEngine.Random.Range(rotationRange1.z, rotationRange2.z) + flipZ;

			// Set the base rotation (world space)
			dummy.transform.rotation = Quaternion.Euler(rRotate);

			// Apply jitter rotation in local space
			...
			if( selectedItem.custom){
				prefabName = selectedItem.customPrefab;
			}
			else{
				if (AssetManager.IDLookup.TryGetValue(selectedItem.prefabID, out string name)){
					prefabName = name;
				}
				else{
					prefabName = "orphan";
            ...
            if (!count.ContainsKey(prefabName))
            {
                count[prefabName] = 0;
                cullcount[prefabName] = 0;
            }
            ...
            if (geo.geologyCollisions.Count>0){
				if (prefabCollisions){
		...
		{
			ConsoleWindow.Instance?.Post($"{count[name]} {name} placed");
			if (cullcount[name] > 0)
			{
				ConsoleWindow.Instance?.Post($"{cullcount[name]} {name} failed physics tests");
			}
		}
		
		if(geo.biomeExclusive){ //(apply terrains)
			
		TerrainPlacement[] terrainPlacements = parentObject.GetComponentsInChildren<TerrainPlacement>();
		if (terrainPlacements.Length > 0)
		{
			int batchSize = 3; // Adjust this value based on performance needs
			for (int i = 0; i < terrainPlacements.Length; i += batchSize)
			{
				int endIndex = Mathf.Min(i + batchSize, terrainPlacements.Length);
				for (int j = i; j < endIndex; j++)
				{
					TerrainPlacement terrainPlacement = terrainPlacements[j];
					TerrainBounds dimensions = new TerrainBounds();
					Vector3 position = terrainPlacement.transform.position;
					Quaternion rotation = terrainPlacement.transform.rotation;
					Vector3 scale = terrainPlacement.transform.localScale;
				...
				yield return null; // Yield after each batch to allow Unity to process other tasks
			}
		}
		else
		{
			Debug.LogWarning($"No TerrainPlacement components found in children of {parentObject.name}");
		}
		
			AddToHeightMap[] heightMapComponents = parentObject.GetComponentsInChildren<AddToHeightMap>();
			if (heightMapComponents.Length > 0)
			{
				int batchSize = 10; // Adjust this value based on performance needs
				for (int i = 0; i < heightMapComponents.Length; i += batchSize)
				{
					int endIndex = Mathf.Min(i + batchSize, heightMapComponents.Length);
					for (int j = i; j < endIndex; j++)
					{
						heightMapComponents[j].Apply();
					}
					yield return null; // Yield after each batch to allow Unity to process other tasks
				}
			}
			...
			{
				Debug.LogWarning($"No AddToHeightMap components found in children of {parentObject.name}");
			}
		}
	...
	public static List<PrefabData> GetRayDataFromPrefab(string prefabPath)
	{
		...
		{
			Debug.Log("Initializing raydata cache");
		...
		if (!File.Exists(fullPath))
		{
			Debug.LogError($"Raydata file not found at: {fullPath}");
		...
		try
		{
			...
			Debug.Log($"Processing {mapInfo.prefabData.Length} possible rays...");
			foreach (var prefabData in mapInfo.prefabData)
			{
				...
				else if (prefabData.id == 1932804458) // Include only specific ID
				{
					rayDataList.Add(prefabData);
				}
			}
			...
			{
				Debug.LogWarning($"No ray prefabs (ID 3244004659) in: {fullPath}");
			}
			else
			{
		...
		{
			Debug.LogError($"Error processing prefab file {fullPath}: {e.Message}\nStackTrace: {e.StackTrace}");
	...
	public static void ClearRayDataCache()
	{
		if (rayDataCache != null)
		{
			rayDataCache.Clear();
			Debug.Log("Ray data cache cleared.");
		}
		else
		{
			Debug.Log("Ray data cache is already null or not initialized.");
		}
	}
...
private static void AdjustRotationForNormalization(ref Vector3 rRotate, GeologyPreset geo, Vector3 normal, Terrain land, int i, int j, int splatRes)
{
    ...
    if (TerrainManager.Slope == null || TerrainManager.Slope.GetLength(0) != TerrainManager.HeightMapRes || TerrainManager.Slope.GetLength(1) != TerrainManager.HeightMapRes)
    {
    ...
    if (TerrainManager.Curvature == null || TerrainManager.Curvature.GetLength(0) != TerrainManager.HeightMapRes || TerrainManager.Curvature.GetLength(1) != TerrainManager.HeightMapRes)
    {
        Debug.LogError($"Curvature map is null or has incorrect dimensions: Expected {TerrainManager.HeightMapRes}x{TerrainManager.HeightMapRes}, Got {(TerrainManager.Curvature != null ? $"{TerrainManager.Curvature.GetLength(0)}x{TerrainManager.Curvature.GetLength(1)}" : "null")}");
        makeCliffMapRunning = false;
        onComplete?.Invoke();
        yield break;
    }
    ...
    if (TerrainManager.SplatMapRes == 0)
    {
		...
		float heightMin = geo.heights.heightMin;
		float heightMax = geo.heights.heightMax;
		...
		Parallel.For(0, TerrainManager.HeightMapRes - 1, i =>
		{
			...
			for (int j = 0; j < TerrainManager.HeightMapRes - 1; j++)
			{
				...
				maskPixels[i * TerrainManager.HeightMapRes + j] = Color.black;
				...
				if (RandomTS.NextDouble() < frequencyInv)
				{
					...
					float curve = TerrainManager.Curvature[i, j] * 300f;
					...
					if (biomeMap[iResRatio, jResRatio, 4] >= 0.1f && !geo.jungle)		{	continue;	}


					lock (spawners) // Ensure thread-safe addition
					{
						spawners.Add(new GeologySpawner
						{
							i = i,
							j = j,
							height = height,
					...
					spawnMap[i, j] = true;
					maskPixels[i * TerrainManager.HeightMapRes + j] = Color.red;
