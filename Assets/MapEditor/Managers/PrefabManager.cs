...
public static class PrefabManager
{
	...
	public static void RuntimeInit()
	{
		...
		Debug.Log(ItemBlacklist.Count + " blacklisted items found");
		
		Entities = new Dictionary<GameObject, bool>();
		/*
        if (DefaultPrefab != null && PrefabParent != null)
        {
            //EditorApplication.update -= OnProjectLoad;
	...
	public static GameObject Setup(GameObject go, string filePath)
	{
		...
		for (int i = 0; i < allComponents.Length; i++)
		{
			...
			if (component is TerrainPathConnect pathConnect)
			{
				terrainPathConnects.Add(pathConnect); // Add to list of TerrainPathConnect components
				continue;
			}
	...
	{
		// Convert ItemBlacklist dictionary to Blacklist object
		Blacklist blacklist = new Blacklist
		{
			blacklistItems = new List<ItemSettings>(PrefabManager.ItemBlacklist.Values)
		};
		...
		// Write to file
		string filePath = Path.Combine(SettingsManager.AppDataPath(), "blacklist.json");
		File.WriteAllText(filePath, json);
		Debug.Log("Blacklist saved to " + filePath + " with " + blacklist.blacklistItems.Count + " items");
	}

	public static Dictionary<string, ItemSettings> LoadBlacklistLookup(Blacklist blacklist)
	{
		Dictionary<string, ItemSettings> blacklistDictionary = new Dictionary<string, ItemSettings>();
		
		foreach (ItemSettings item in blacklist.blacklistItems)
		{
			// Ensure the path is not null or empty to avoid invalid keys
			if (!string.IsNullOrEmpty(item.path))
			{
				// Normalize path to use forward slashes for consistency
				string normalizedPath = item.path.Replace("\\", "/");
				blacklistDictionary[normalizedPath] = item;
			}
		}
		
		return blacklistDictionary;
	}
	
	public static void LoadBlacklistFromJson(string filePath)
	{
		if (!File.Exists(filePath))
		{
			ItemBlacklist = new Dictionary<string,ItemSettings>();
		}
	...
	public static void CreateBlacklist(){
		...
		// Code to generate JSON with the specified entries
		Blacklist blacklist = new Blacklist
		{
			blacklistItems = new List<ItemSettings>
			{
				new ItemSettings
				{
		...
		string json = JsonConvert.SerializeObject(blacklist, Formatting.Indented);
		File.WriteAllText(Path.Combine(SettingsManager.AppDataPath() , "blacklist.json"), json);
	}
	...
	public static void Spawn(GameObject go, PrefabData prefabData, Transform parent)
	{
		try{
			...
			// Handle rotation explicitly
			if (prefabData.rotation == null)
			{
				// Use a dummy (0,0,0) rotation
	...
	public static bool inTerrain(PrefabData prefab, List<PrefabData> rayData)
	{
		...
		{
			// Apply prefab's scale to the ray position
			Vector3 scaledRayPosition = new Vector3(
				rayPrefab.position.x * prefab.scale.x,
				rayPrefab.position.y * prefab.scale.y,
				rayPrefab.position.z * prefab.scale.z
			);

			// Rotate the scaled ray position
			Vector3 rotatedRayPosition = prefabRotation * additionalRotation * scaledRayPosition;

			// Calculate ray origin, including PrefabParent position
			Vector3 rayOrigin = prefab.position + rotatedRayPosition + PrefabParent.position;

			// Combine prefab rotation with rayPrefab rotation
			Quaternion rayPrefabRotation = Quaternion.Euler(rayPrefab.rotation.x, rayPrefab.rotation.y, rayPrefab.rotation.z);
			Quaternion combinedRotation = prefabRotation * rayPrefabRotation;

			// Apply scale to ray direction (use prefab's y-scale for vertical rays)
			Vector3 rayDirection = combinedRotation * Vector3.up;
			float rayLength = 3f * rayPrefab.scale.y * prefab.scale.y; // Scale ray length by both ray and prefab y-scale
			bool rayhit = false;
			uint rayID = rayPrefab.id;
			
			//jungle walkway metal pillar A
			if(rayID == 1932804458)
			{
				//alternate logic
				//this prefab is longer than the other
				rayLength *=1.26f;
				//the test for this returns false if it hits land (not in terrain)
				if (Physics.Raycast(rayOrigin, rayDirection, out RaycastHit hitInverse, rayLength, groundMask))
				{
					return false;
				}        
				else        {
			...
			// Perform raycast with scaled length
			if (Physics.Raycast(rayOrigin, rayDirection, out RaycastHit hit, rayLength, groundMask))
			{
				rayhit = true;
			}        
			else        {
			...
			Debug.DrawRay(rayOrigin, rayDirection * rayLength, rayColor, 1.5f);
		}
...
[ConsoleCommand("count entities")]
public static void CountEntities()
{
    // Ensure Entities dictionary is not null
    if (Entities == null)
    {
    ...
    // Count non-null GameObjects in Entities
    int count = Entities.Keys.Count();
    
    Debug.LogError(count + " entities found\n");
}

[ConsoleCommand("list entities")]
public static void ListEntities()
{
    ...
    // Collect string names of entities from non-null GameObjects
    foreach (var go in Entities.Keys)
    {
        if (go != null)
        {
            PrefabDataHolder holder = go.GetComponent<PrefabDataHolder>();
            if (holder != null && AssetManager.IDLookup.TryGetValue(holder.prefabData.id, out string name))
            {
    ...
    // Output entity names and count
    if (entityNames.Count == 0)
    {
        Debug.LogError("No entities found\n");
    }
    else
    {
        Debug.Log("Entities found:\n" + string.Join("\n", entityNames) + "\nTotal: " + entityNames.Count + " entities\n");
    }
}

// Refactored ConsoleCommand to queue deletions
[ConsoleCommand("delete non-entities")]
public static void DeleteNonEntities()
{
    // List to queue prefabs for deletion
    List<PrefabDataHolder> toDelete = new List<PrefabDataHolder>();
    int count = 0;
    ...
    // Iterate and queue non-entities for deletion
    for (int k = 0; k < CurrentMapPrefabs.Length; k++)
    {
        if (CurrentMapPrefabs[k] != null && CurrentMapPrefabs[k].gameObject != null)
        {
            // Check if the prefab's gameObject is NOT in the Entities dictionary
            if (!Entities.ContainsKey(CurrentMapPrefabs[k].gameObject))
            {
                toDelete.Add(CurrentMapPrefabs[k]);
                count++;
            }
        }
    }
    ...
    foreach (var prefab in toDelete)
    {
        if (prefab != null && prefab.gameObject != null)
        {
            ...
            // Null the entry in CurrentMapPrefabs
            for (int k = 0; k < CurrentMapPrefabs.Length; k++)
            {
                if (CurrentMapPrefabs[k] == prefab)
                {
                    CurrentMapPrefabs[k] = null;
                    break;
                }
            }
        }
    ...
    Debug.LogError(count + " non-entities removed\n");
	...
	public static void deletePrefabIDs(PrefabDataHolder[] prefabs, uint ID)
	{
		...
		for (int k = 0; k < prefabs.Length; k++)
		{
			if (prefabs[k] != null)
			{
				...
				{			
								GameObject.Destroy(prefabs[k].gameObject);
								prefabs[k] = null;
								count ++;
				}
			}
...
public static class Coroutines
	...
	public static IEnumerator SpawnPrefabs(PrefabData[] prefabs, int progressID)
	{
		...
		if (prefabs.Length == 0)
		{
			LoadScreen.Instance.SetMessage("Prefab loading completed");
			LoadScreen.Instance.Progress(0f);
