...
public static class BindManager
{
    ...
    {
        Debug.Log("setting default binds including camera slots");
        binds = new List<Bind>();
        ...
        // === SPECIAL CONTROLS ===
        binds.Add(new Bind("transparencyToggle", InputActionType.Button, "<Keyboard>/f", false, false, false));
        ...
        // === CAMERA SLOTS (0-9) - SAVE ===
        for (int i = 0; i <= 9; i++)
        {
            string keyCode = i == 0 ? "<Keyboard>/0" : $"<Keyboard>/{i}";
            binds.Add(new Bind($"saveCameraSlot{i}", InputActionType.Button, keyCode, true, false, false)); // Ctrl + 0-9
        }

        // === CAMERA SLOTS (0-9) - LOAD ===
        for (int i = 0; i <= 9; i++)
        {
            string keyCode = i == 0 ? "<Keyboard>/0" : $"<Keyboard>/{i}";
            binds.Add(new Bind($"loadCameraSlot{i}", InputActionType.Button, keyCode, false, false, false)); // Just 0-9
        }
        ...
        Debug.Log("default binds initialized (including camera slots)");
        SettingsManager.SaveSettings();
    }
