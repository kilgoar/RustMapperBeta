...
public class PostBuildProcessor
{
    ...
    public static void BuildGame()
    {
        ...
        string appDataPath = SettingsManager.AppDataPath();
	...
	public static void BuildRelease()
    {
		...
		string buildInfoPath = Path.Combine(releasePath, "buildinfo");
        ...
        SettingsManager.CopyEditorSettings("E:/RustMapper/");
		
		    // Generate and save build info
		try
		{
			string buildId = $"Build_{DateTime.UtcNow:yyyyMMdd_HHmm}_{Guid.NewGuid().ToString("N").Substring(0, 8)}";
			File.WriteAllText(buildInfoPath, buildId);
			UnityEngine.Debug.Log($"Saved build info: {buildId} to {buildInfoPath}");
		}
		catch (Exception ex)
		{
			UnityEngine.Debug.LogError($"Failed to save build info: {ex.Message}");
		}
