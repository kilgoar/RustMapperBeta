...
public class DebugManager : MonoBehaviour
{
    ...
    private void Awake()
    {
		...
		// Read build info from the program root directory
        string programRoot = Directory.GetCurrentDirectory();
        string buildInfoPath = Path.Combine(programRoot, "buildinfo");
        try
        {
            if (File.Exists(buildInfoPath))
            {
                buildId = File.ReadAllText(buildInfoPath).Trim();
                Debug.Log($"Build ID: {buildId}");
            }
            else
            {
                buildId = "Unknown";
                Debug.LogWarning("Build info file not found at " + buildInfoPath);
            }
        }
        catch (Exception ex)
        {
            buildId = "Error";
            Debug.LogError($"Failed to read build info: {ex.Message}");
        }
    }
    ...
    public void Update(){
        ...
        usedMemoryf = usedMemory / (1024f * 1024f);
            ...
            fpsReadout.text = 
							  ...
							  $"{framerate:F1} fps | {usedMemoryf:F1} MB" + $" --BuildID: {buildId}\n";
