...
public static class TopologyData
{
    ...
    public static void UpdateTexture()
    {
		...
		if (TopologyTexture == null || TopologyTexture.width != resolution || TopologyTexture.height != resolution || TopologyTexture.format != TextureFormat.RGBA32)
        {
            TopologyTexture = new Texture2D(resolution, resolution, TextureFormat.RGBA32, false, true);
			TopologyTexture.filterMode = FilterMode.Point; 
			Debug.Log("topology texture initializing");
            UpdateTexture(); // Populate initial pixel data
        }
		Shader.SetGlobalTexture("Terrain_Topologies", TopologyTexture);
        ...
        Parallel.For(0, resolution, i =>
        {
            for (int j = 0; j < resolution; j++)
            {
				...
				pixels[i * resolution + j] = BitUtility.EncodeInt(value);
            }
        });

        TopologyTexture.SetPixels32(pixels);
...
public static void SetTopologyRegion(int[,] topologyValues, int x, int y, int width, int height)
{
    ...
    // Update the specified region of the topology map and Topology array additively
    Parallel.For(0, height, i =>
    {
        for (int j = 0; j < width; j++)
        {
            ...
            if (x + j < topologyMap.res && y + i < topologyMap.res)
            {
                ...
                int existingValue = topologyMap[y + i, x + j];
                int updatedValue = existingValue | newValue; // Apply additively
                topologyMap[y + i, x + j] = updatedValue;

                // Update Topology array for each layer
                for (int layerIndex = 0; layerIndex < TerrainTopology.COUNT; layerIndex++)
                {
                    ...
                    bool wasLayerActive = (existingValue & layerBit) != 0;

                    // Only update if the layer state changes
                    if (isLayerActive != wasLayerActive)
                    {
                        Topology[layerIndex][y + i, x + j, 0] = isLayerActive ? 1f : 0f; // Set layer
                        Topology[layerIndex][y + i, x + j, 1] = isLayerActive ? 0f : 1f; // Set inverse
                    }
                }
            }
        }
...
public static void OverwriteTopologyRegion(int[,] topologyValues, int x, int y, int width, int height)
{
	...
	public static void AddTopology(int layer, int x, int y, int width, int height, bool[,] bitmap)
	{
		TerrainMap<int> topologyMap = GetTerrainMap();

		if (bitmap == null || bitmap.GetLength(0) != height || bitmap.GetLength(1) != width)
		{
			return;
		}

		int layerIndex = TerrainTopology.TypeToIndex(layer);
				
		Parallel.For(0, height, i =>
		{
			for (int j = 0; j < width; j++)
			{
				if (bitmap[i, j] && x + j < topologyMap.res && y + i < topologyMap.res)
				{
					topologyMap[y + i, x + j] |= layer; // Set the bit for this layer
					Topology[layerIndex][y + i, x + j, 0] = 1f; // Set layer
                    Topology[layerIndex][y + i, x + j, 1] = 0f; // Clear inverse
				}
			}
		});
	...
	public static void EraseTopology(int layer, int x, int y, int width, int height, bool[,] bitmap)
	{
		TerrainMap<int> topologyMap = GetTerrainMap();
		...
		Parallel.For(0, height, i =>
		{
			for (int j = 0; j < width; j++)
			{
				...
				{
					topologyMap[y + i, x + j] &= ~layer; // Unset the bit for this layer
					Topology[layerIndex][y + i, x + j, 0] = 0f; // Clear layer
                    Topology[layerIndex][y + i, x + j, 1] = 1f; // Set inverse
				}
			}
		});
