Shader "Custom/Rust/StandardTerrain"
{
	...
	BiomeColors[38] = Splat6_JungleColor; BiomeColors[39] = Splat7_JungleColor;
		...
		alpha = UNITY_SAMPLE_TEX2D(Terrain_Alpha, IN.tc_Control0).r;
		...
		if (_PreviewMode < -.5)		{
			float blendMap = UNITY_SAMPLE_TEX2D(Terrain_BlendMap, IN.tc_Control0).a;
			albedo.rgb *= blendMap;
		}
        ...
        if (_PreviewMode != 0.0)
            {
				...
				if (_PreviewMode > -1.5 && _PreviewMode < -0.5) // monument
                {
                    albedo.rgb = lerp(albedo.rgb, float3(0.5, 0.1, 0.8), influence); // purp
                }				
                else if (_PreviewMode > 0.5 && _PreviewMode < 1.5) // Splat, Topology, Alpha (1.0)
                {
                ...
                {
                    albedo.rgb = lerp(albedo.rgb, float3(0.1, 0.1, 0.8), influence); // Blue
                }
				...
				else if (_PreviewMode > 6.5 && _PreviewMode < 7.5) // Flatten (6.0)
                {
                    albedo.rgb = lerp(albedo.rgb, float3(0.0, 0.8, 0.7), influence); // idk
                }
            }
		
			if (_LandMask > 0.0)
			{
				float maskValue = UNITY_SAMPLE_TEX2D(Terrain_Preview, IN.tc_Control0).r;
				float3 brightBlue = float3(0.0, 0.7, 1.0); // Bright blue color
				albedo.rgb = lerp(albedo.rgb, brightBlue, maskValue);
